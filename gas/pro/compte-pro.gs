...
function onOpen() {
  ...
  var entries = [
    ...
    {
      name : "Export Tiers Payant pour BNC Express",
      functionName : "exportTiersPayant"
    },
    {
      name : "Import CCM",
...
/**
 * Exporte au format BNC Express les écritures correspondant à la catégorie "Tiers payant, ALD & Maison retraite & co"
 */
function exportTiersPayant() {
  ...
  var data = currentSheet.getRange(5, 1, 250, 9).getValues();
  var exportedData = [];
  ...
  for (var i = 0; i<data.length; i++) {
    ...
    if (isExportOn) {
      ...
      if (checked == 'ok' && !inBNC) {
        ...
        rowExp.push(date);
        if (libelle.startsWith('C.R.P.C.E.N') || libelle.startsWith('CPAM') || libelle.startsWith('MGEN') || libelle.startsWith('MSA') || libelle.startsWith('MGP')) {
          rowExp.push('Retour Noémie - ' + libelle);
        } else {
          rowExp.push(libelle);
        }
        ...
        var numCompte = '706100';
        
        rowExp.push(numCompte);
        indexes.push(i + firstDataLineNum);
      }
    } else if (libelle == 'Tiers payant, ALD & Maison retraite & co') {
        isExportOn = true;
    }
  }
